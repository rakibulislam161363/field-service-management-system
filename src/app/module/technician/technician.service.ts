...
const createTechnician = async (payload: CreateTechnicianPayload) => {
  ...
  if (user.role !== "TECHNICIAN") {
    throw new AppError(400, "Only users with technician role can have a technician profile");
  }
...
const updateTechnician = async (
  ...
  if (role === "TECHNICIAN" && technician.userId !== userId) {
    throw new AppError(403, "You are not allowed to update this technician profile");
  }
...
const addSkill = async (
  ...
  const existingSkill = await prisma.technicianSkill.findUnique({
    where: { technicianId_skillId: { technicianId: id, skillId: payload.skillId } },
  });
  if (existingSkill) throw new AppError(409, "Skill is already assigned to this technician");
...
const removeSkill = async (id: string, userId: string, role: string, skillId: string) => {
  const technician = await prisma.technicianProfile.findUnique({ where: { id } });
  if (!technician) throw new AppError(404, "Technician profile not found");
  if (role === "TECHNICIAN" && technician.userId !== userId) {
    throw new AppError(403, "You are not allowed to update this technician profile");
